// server/test-p6-swagger-endpoint.js
// Test Oracle P6 API based on Swagger documentation
// According to Oracle P6 REST API, the endpoint is GET /restapi/project with fields parameter

const axios = require('axios');

const REAL_TOKEN = 'eyJ4NXQjUzI1NiI6IlV6LU1BTlgyS0VncEFpb2I3cEVwQlZWSmtZSzFvV2FRczBacHhMbDI5NWciLCJ4NXQiOiJGNmE4X1lJMENCTEI3LVpkd3RWNjM5bXFqZ0kiLCJraWQiOiJTSUdOSU5HX0tFWSIsImFsZyI6IlJTMjU2In0..yQ9LrwkWiYT18s9n4NuvVgLyQIKndDwrVcmITzZgy_bawNcGp9D7R40AMZ31mvZCeL52eNXlTKwkKWjdao2yWtGZrhPIybJK9ahjtgZsCucX0iNpT8gBjlAyW7VGY6F5wSQyNKn1JxSuCEqkIiAFO996ibnFN4UDxsrA2af3MmRt8RmeuH6qYo8uF2-Aux8JkerzNaW4z4EUoWbW-SQhXbL1UNJO3ZGesq8tjYaU4uXKdAIyL7L8fUbjCeVr1yPsZziZLa75CSnjXRwmX_cLBMn5pzg7DcKiHkbu6TqjuViIOfJDiDQ46IfLByWgLkB-vPj4TPX1dP48RvMxKGsCuA';
const BASE_URL = 'https://sin1.p6.oraclecloud.com/adani/stage/p6ws';

async function testSwaggerEndpoint() {
    console.log('Testing Oracle P6 API based on Swagger documentation...\n');

    // Based on Oracle P6 REST API documentation, try with proper field names
    // Common Oracle P6 Project fields from documentation
    const testQueries = [
        // Try with basic fields
        '?fields=ObjectId',
        '?fields=Name',
        '?fields=Id',
        // Try with multiple fields
        '?fields=ObjectId&fields=Name',
        '?fields=Id&fields=Name',
        // Try comma-separated
        '?fields=ObjectId,Name',
        '?fields=Id,Name',
        // Try with limit
        '?limit=5&fields=ObjectId,Name',
        '?limit=5&fields=Id,Name',
        // Try without fields but with limit
        '?limit=1'
    ];

    for (const query of testQueries) {
        try {
            console.log(`Testing: GET /restapi/project${query}`);
            const response = await axios.get(`${BASE_URL}/restapi/project${query}`, {
                headers: {
                    'Authorization': `Bearer ${REAL_TOKEN}`,
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
                },
                timeout: 15000
            });

            console.log('✓ SUCCESS!');
            console.log('Status:', response.status);
            console.log('Response:', JSON.stringify(response.data, null, 2).substring(0, 2000));
            console.log('');
            console.log('='.repeat(80));
            console.log(`WORKING QUERY: ${query}`);
            console.log('='.repeat(80));
            return;

        } catch (error) {
            if (error.response) {
                console.log(`✗ HTTP ${error.response.status}`);
                if (error.response.data && error.response.status !== 404) {
                    console.log('Error:', JSON.stringify(error.response.data).substring(0, 150));
                }
            } else {
                console.log(`✗ ${error.message}`);
            }
            console.log('');
        }
    }

    console.log('All queries failed. The API might require authentication setup or different parameters.');
}

testSwaggerEndpoint();
